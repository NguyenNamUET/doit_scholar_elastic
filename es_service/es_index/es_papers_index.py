...
PAPER_METADATA_PATH = "/home/nguyennam/Downloads/data/20201115_162336"
def index_paper_document(path):
    try:
        paper_document = load_jsonl_from_gz(path)
...
def index_paper_document_v2(path):
    try:
        paper_document = load_jsonl_from_gz(path)
        #Calculate h-index of each author
        insert_doc(es=elasticsearch_connection, index=PAPER_DOCUMENT_INDEX,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        for sitemap_dir in os.listdir(PAPER_METADATA_PATH):
            ...
            for index, paths_group in enumerate(papers_group):
                future_to_path = {executor.submit(index_paper_document_v2, PAPER_METADATA_PATH+"/"+sitemap_dir+"/"+path): path
                                  for path in paths_group if path is not None}
                # Just ignore this
