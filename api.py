...
from es_service.es_search.es_search_paper import get_paper_by_id, get_all_papers, get_all_fields_of_study, \
    search_paper_title, search_paper_abstract, get_all_topics, get_paper_by_fos, get_paper_by_title_and_fos
...
def getPaperByTopic(query: paperItem):
    result = get_paper_by_fos(es=elasticsearch_connection,
                              index=PAPER_DOCUMENT_INDEX,
                              ...
                              source=query.source,
                              sort_by=query.sort_by,
                              is_should=query.topic_is_should)[1]
    return result


@app.post("/s2api/papers/getPaperByTopic")
def getPaperByTopic(query: paperItem):
    result = get_paper_by_title_and_fos(es=elasticsearch_connection,
                                        index=PAPER_DOCUMENT_INDEX,
                                        fields_of_study=query.topics,
                                        start=query.start,
                                        size=query.size,
...
@app.post("/s2api/papers/getPaperByTitleAndFOS")
def getPaperByTitleAndFOS(query: paperItem):
    result = get_paper_by_title_and_fos(es=elasticsearch_connection,
